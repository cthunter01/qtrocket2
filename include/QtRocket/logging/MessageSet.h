...
/// change the set's contents throws std::logic_error. As in Java the check happens only when
/// something would change: remove() of an absent message, filterOut() without a match, clear()
/// of an empty set and addAll() of an empty set return quietly. Assignment is not a change of
/// the contents but a replacement of the whole set, like reassigning the reference in Java: it
/// overwrites an immuted set and carries over the source's mutability (so a move assignment can
/// stay noexcept). Hold a set const, or hand it out by const reference, to forbid that. A copy
/// or a move of a set keeps its immutability.
///
...
/// instead. Iterator.remove() is erase(). The ModID that Java bumps on add() waits for
/// util/ModId.h. Java's findById() logs an error when nothing is found; here it just returns
/// nullptr.
///
...
public:
    ...
    /// Replaces the whole set, immutability included (see the class comment).
    MessageSet& operator=(MessageSet&&) noexcept = default;
    ~MessageSet()                                = default;
    ...
    [[nodiscard]] const T* findById(std::string_view id) const noexcept;
    [[nodiscard]] T*       findById(std::string_view id) noexcept;
    ...
    /// Makes the set immutable for good; repeated calls do nothing (Java: immute()).
    void               immute() noexcept { m_mutable = false; }
...
template <class T>
    requires std::derived_from<T, Message>
MessageSet<T>::MessageSet(const MessageSet& other) : m_mutable(other.m_mutable)
{
...
template <class T>
    requires std::derived_from<T, Message>
const T* MessageSet<T>::findById(std::string_view id) const noexcept
{
    const auto it = std::ranges::find_if(
...
template <class T>
    requires std::derived_from<T, Message>
T* MessageSet<T>::findById(std::string_view id) noexcept
{
    const auto it = std::ranges::find_if(
        m_messages, [id](const std::unique_ptr<T>& m) { return m->id() == id; });
    return it == m_messages.end() ? nullptr : it->get();
...
{
    // TODO(util): BugError once util/BugError.h exists (Java: IllegalStateException).
    if (!m_mutable)
    {
        throw std::logic_error("MessageSet has been made immutable");
    }
}
...
std::unique_ptr<T> MessageSet<T>::cloneMessage(const T& message)
{
    ...
    if (typed == nullptr)
    {
        throw std::logic_error("Message::clone() returned a message of another type");
    }
