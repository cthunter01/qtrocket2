...
{
protected:
    MaterialStorageTest() { addBuiltinMaterials(m_storage); }
...
{
    return found.value_or(Material::newMaterial(Type::CUSTOM, "<not found>", 0, true));
}

/// Every material compares at or below the next one (Material::compareTo).
void expectSortedByName(const MaterialDatabase& database)
{
    for (std::size_t i = 1; i < database.size(); i++)
    {
        EXPECT_LE(database.get(i - 1).compareTo(database.get(i)), 0) << i;
    }
}
...
TEST(MaterialStorage, StartsEmpty)
{
    ...
    EXPECT_THROW(static_cast<void>(empty.materialCount(Type::CUSTOM)), BugError);
}

TEST_F(MaterialStorageTest, DatabasesByType)
{
    EXPECT_EQ(&m_storage.database(Type::BULK), &m_storage.bulkMaterials());
    EXPECT_EQ(&m_storage.database(Type::SURFACE), &m_storage.surfaceMaterials());
    EXPECT_EQ(&m_storage.database(Type::LINE), &m_storage.lineMaterials());
    EXPECT_THROW(static_cast<void>(m_storage.database(Type::CUSTOM)), BugError);
    EXPECT_EQ(m_storage.materialCount(Type::BULK), 32U);
    EXPECT_EQ(m_storage.materialCount(Type::SURFACE), 8U);
    EXPECT_EQ(m_storage.materialCount(Type::LINE), 42U);
...
TEST_F(MaterialStorageTest, FindByNameIgnoresCase)
{
    ...
    EXPECT_FALSE(orNotFound(m_storage.findMaterial(Type::BULK, "Aluminum")).isDocumentMaterial());
}
...
TEST_F(MaterialStorageTest, FindByNameAndDensityOrFallBackToACustomMaterial)
{
    ...
    const Material found =
        m_storage.findMaterial(Type::BULK, "ALUMINUM", 2700 + 1e-6, std::nullopt);
    ...
    EXPECT_EQ(m_storage.findMaterial(Type::BULK, "Aluminum", 2700, MaterialGroup::WOODS).getGroup(),
              MaterialGroup::METALS);
    EXPECT_EQ(m_storage.findMaterial(Type::BULK, "Aluminum", 2700).getGroup(),
              MaterialGroup::METALS);

    // Another density is another material: a new user-defined document material.
    const Material other = m_storage.findMaterial(Type::BULK, "Aluminum", 2710, std::nullopt);
    ...
    EXPECT_EQ(m_storage.totalMaterialCount(), 82U);  // the fallback is not stored

    const Material unknown =
        m_storage.findMaterial(Type::LINE, "Unknownium", 0.0042, MaterialGroup::NYLONS);
...
TEST_F(MaterialStorageTest, FindWithShearModulus)
{
    ...
    const Material aluminum =
        m_storage.findMaterial(Type::BULK, "Aluminum", 2700, 26.0e9, MaterialGroup::METALS);
    EXPECT_FALSE(aluminum.isUserDefined());
    EXPECT_EQ(aluminum.getInPlaneShearModulus(), 26.0e9);
    const Material within =
        m_storage.findMaterial(Type::BULK, "aluminum", 2700, 26.0e9 * (1 + 1e-9), std::nullopt);
    EXPECT_FALSE(within.isUserDefined());

    const Material differentShear =
        m_storage.findMaterial(Type::BULK, "Aluminum", 2700, 0.0, std::nullopt);
    ...
    const Material withGroup =
        m_storage.findMaterial(Type::BULK, "Aluminum", 2700, 25.0e9, MaterialGroup::METALS);
    ...
    EXPECT_FALSE(
        m_storage.findMaterial(Type::LINE, "Thread (heavy-duty)", 0.0003, 0.0, std::nullopt)
            .isUserDefined());
    EXPECT_THROW(
        static_cast<void>(m_storage.findMaterial(Type::CUSTOM, "x", 1.0, 0.0, std::nullopt)),
        BugError);
}
...
TEST_F(MaterialStorageTest, AddAndRemoveMaterials)
{
    ...
    EXPECT_TRUE(m_storage.addMaterial(user));
    EXPECT_EQ(m_storage.bulkMaterials().size(), 33U);
    EXPECT_TRUE(m_storage.bulkMaterials().contains(user));
    EXPECT_FALSE(m_storage.addMaterial(user));  // already there
    EXPECT_EQ(orNotFound(m_storage.findMaterial(Type::BULK, "unobtainium")).getDensity(), 42);
    EXPECT_FALSE(m_storage.findMaterial(Type::BULK, "Unobtainium", 42).isDocumentMaterial());

    EXPECT_TRUE(m_storage.removeMaterial(user));
    EXPECT_EQ(m_storage.bulkMaterials().size(), 32U);
    EXPECT_FALSE(m_storage.removeMaterial(user));
    EXPECT_EQ(m_storage.findMaterial(Type::BULK, "Unobtainium"), std::nullopt);

    const Material line = Material::newMaterial(Type::LINE, "String", 0.001, true, true);
    EXPECT_TRUE(m_storage.addMaterial(line));
    EXPECT_EQ(m_storage.lineMaterials().size(), 43U);
    EXPECT_EQ(m_storage.bulkMaterials().size(), 32U);
    EXPECT_TRUE(m_storage.removeMaterial(line));

    const Material custom = Material::newMaterial(Type::CUSTOM, "Odd", 1.0, true);
    EXPECT_THROW(static_cast<void>(m_storage.addMaterial(custom)), BugError);
    EXPECT_THROW(static_cast<void>(m_storage.removeMaterial(custom)), BugError);
    EXPECT_EQ(m_storage.totalMaterialCount(), 82U);
}
...
TEST_F(MaterialStorageTest, SignalsUserMaterialsToThePreferences)
{
    ...
    m_storage.userMaterialAdded.connect(
        [&added](const Material& m) { added.push_back(m.getName()); });
    m_storage.userMaterialRemoved.connect(
    ...
    m_storage.addMaterial(Material::newMaterial(Type::BULK, "Mine", 2.0, true));
    m_storage.addMaterial(Material::newMaterial(Type::SURFACE, "Cloth", 0.1, true, true));
    m_storage.addMaterial(
        Material::newMaterial(Type::LINE, "Cord", 0.01, MaterialGroup::NYLONS, true));
    EXPECT_EQ(added, (std::vector<std::string>{"Mine", "Cloth", "Cord"}));
    m_storage.addMaterial(Material::newMaterial(Type::BULK, "Mine", 2.0, true));  // rejected
    EXPECT_EQ(added.size(), 3U);

    // Every removal is announced, user-defined or not.
    m_storage.removeMaterial(
        Material::newMaterial(Type::BULK, "System", 1.0, MaterialGroup::OTHER, false));
    m_storage.removeMaterial(
        Material::newMaterial(Type::LINE, "Cord", 0.01, MaterialGroup::NYLONS, true));
    m_storage.removeMaterial(
    ...
    m_storage.surfaceMaterials().remove(
        Material::newMaterial(Type::SURFACE, "Cloth", 0.1, true, true));
    EXPECT_EQ(removed.size(), 3U);
    m_storage.bulkMaterials().add(Material::newMaterial(Type::BULK, "Direct", 3.0, true));
    EXPECT_EQ(added.back(), "Direct");
}

TEST_F(MaterialStorageTest, AllMaterialsIsAnIndependentSortedCopy)
{
    MaterialDatabase all = m_storage.allMaterials();
    EXPECT_EQ(all.size(), 82U);
    expectSortedByName(all);
    EXPECT_EQ(all.get(0).getName(), "ABS - 100% infill");
    EXPECT_TRUE(all.contains(orNotFound(m_storage.findMaterial(Type::SURFACE, "Mylar"))));
